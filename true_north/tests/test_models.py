...
def test_compute_next_send_send_time_tomorrow_when_already_past():
    """A schedule created after send_time has passed should fire TOMORROW."""
    from datetime import datetime as _dt
    from datetime import time, timedelta
    from unittest.mock import patch

    tz = timezone.get_current_timezone()
    # Simulate "now" as 09:10 local time so that 09:00 has already passed.
    now_local_date = timezone.now().astimezone(tz).date()
    now_utc = timezone.make_aware(
        _dt.combine(now_local_date, time(9, 10, 0)), tz
    )
    ...
    assert local_result.date() == (now_local_date + timedelta(days=1))
    assert local_result.hour == 9
    assert local_result.minute == 0


def test_compute_next_send_days_of_week_uses_local_date():
    """days_of_week branch should use the local date, not the UTC date."""
    from datetime import datetime as _dt
    from datetime import time, timedelta
    from unittest.mock import patch

    tz = timezone.get_current_timezone()
    # Simulate "now" as local Tuesday 11:00 PM with send_time 23:30 (still future).
    # Use a known Tuesday; find the next Tuesday from today first.
    today_local = timezone.now().astimezone(tz).date()
    days_until_tuesday = (1 - today_local.weekday()) % 7  # 1 = Tuesday
    next_tuesday = today_local + timedelta(days=days_until_tuesday)

    now_utc = timezone.make_aware(
        _dt.combine(next_tuesday, time(23, 0, 0)), tz
    )

    send_time = time(23, 30)
    schedule = CoreValueEmailScheduleFactory.build(
        frequency=CoreValueEmailSchedule.DAILY,
        send_time=send_time,
    ...
    local_result = result.astimezone(tz)
    # Should fire THIS Tuesday at 23:30 (not next Tuesday), since 23:30 > 23:00.
    assert local_result.date() == next_tuesday
